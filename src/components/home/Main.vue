...
<script>
    export default {
        ...
        methods: {
            onClickShortener(){
                if(this.url.length < 1) return this.$toasted.show('URL을 입력하세요');
                if (!(this.url.indexOf("http://") == 0 || this.url.indexOf("https://") == 0)) return this.$toasted.show('URL형식을 확인하세요');
                this.shortUrl = ''
                this.originUrl = ''
                this.axios
                .post(`/register.json?url=${this.url}`)
                .then((res)=>{
                    if(res.data.url){
                        this.$toasted.success('URL 축약 성공');
